...
# in the past
def load_images_from_file(file_path, image_count=128):
    img = Image.open(file_path).convert('RGB')  
    img_width, img_height = img.size
    single_width = img_width // 8
    single_height = img_height // 16
    images = []
...
directory_path = 'reports_WGAN64_CelebA/images'
average_scores = []
# all_scores  = []
# process each file
for file_name in sorted(os.listdir(directory_path)):
    ...
    if file_path.endswith('.png'):
        ...
        data_loader = torch.utils.data.DataLoader(image_tensors, batch_size=128)
...
# dir
output_dir = 'reports_WGAN64_CelebA/ISScore'
