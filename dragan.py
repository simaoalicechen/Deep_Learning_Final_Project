...
def compute_gradient_penalty(D, X):
    ...
    alpha = Tensor(np.random.random(size=X.shape)).cuda() if cuda else Tensor(np.random.random(size=X.shape))

    # interpolates = alpha * X + ((1 - alpha) * (X + 0.5 * X.std() * torch.rand(X.size())))
    interpolates = alpha * X + ((1 - alpha) * (X + 0.5 * X.std() * torch.rand(X.size()).cuda() if cuda else torch.rand(X.size())))
