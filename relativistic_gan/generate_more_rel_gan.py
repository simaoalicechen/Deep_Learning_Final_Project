...
optimizer_G.load_state_dict(checkpoint['gen_opt'])
optimizer_D.load_state_dict(checkpoint['dis_opt'])
Tensor = torch.cuda.FloatTensor if torch.cuda.is_available() else torch.FloatTensor

for i in range(60):
    z = Variable(Tensor(np.random.normal(0, 1, (64, 100))))
    gen_imgs = generator(z)
    save_image(gen_imgs.data[:25], "images/%d.png" % i, nrow=5, normalize=True)